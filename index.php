...
if ($mybb->input['board']) {
    ...
    if ($board->requires_loginconvert == true) {
        ...
        if (!file_exists(MYBB_ROOT . "inc/plugins/loginconvert.php")) {
            ...
            echo "			<input type=\"hidden\" name=\"board\" value=\"" . htmlspecialchars_uni(
                    $mybb->input['board']
                ) . "\" />";
...
if (!$import_session['first_page'] && !$mybb->input['first_page']) {
    ...
    $output->print_footer(
        ...
        false,
        $lang->next,
        "id=\"main_submit_button\" disabled=\"disabled\"",
...
if ($mybb->input['requirements_check'] == 1 && $import_session['requirements_pass'] == 1 && $mybb->request_method == "post") {
    ...
    if (!$import_session['requirements_check'] || ($mybb->input['first_page'] == 1 && $mybb->request_method == "post") || !$import_session['requirements_pass']) {
        ...
        if (!empty($contents)) {
            ...
            $latest_version = "<strong>" . htmlspecialchars_uni(
                    $tree['mybb_merge']['latest_version']['value']
                ) . "</strong> (" . $latest_code . ")";
