...
class VANILLA_Converter_Module_Threads extends Converter_Module_Threads
{
        ...
        while ($thread = $this->old_db->fetch_array($query)) {
            ...
            $post = [
                ...
                "subject" => $db->escape_string(
                    encode_to_utf8($this->bbcode_parser->convert_title($thread['Name']), "discussion", "posts")
                ),
                ...
                "message" => $db->escape_string(
                    encode_to_utf8($this->bbcode_parser->convert($thread['Body']), "discussion", "posts")
                ),
    ...
    function convert_data($data)
    {
        ...
        $insert_data['subject'] = encode_to_utf8(
            $this->bbcode_parser->convert_title($data['Name']),
            "discussion",
            "threads"
        );
