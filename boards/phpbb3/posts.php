...
class PHPBB3_Converter_Module_Posts extends Converter_Module_Posts
{
    ...
    function convert_data($data)
    {
        ...
        $insert_data['subject'] = encode_to_utf8(
            $this->bbcode_parser->convert_title($data['post_subject']),
            "posts",
            "posts"
        );
        ...
        $insert_data['message'] = strip_tags(
            encode_to_utf8(
                $this->bbcode_parser->convert($data['post_text'], $data['bbcode_uid'], $data['post_id']),
                "posts",
                "posts"
            )
        );
