...
class PHPBB3_Converter_Module_Forumperms extends Converter_Module_Forumperms
{
    ...
    function pre_setup($cache = true)
    {
        $query = $this->old_db->simple_select(
            "acl_options",
            "auth_option_id, auth_option",
            "auth_option IN ('" . implode("','", $this->convert_val) . "')"
        );
        ...
        $query = $this->old_db->simple_select(
            "acl_roles_data",
            "DISTINCT(role_id)",
            "auth_option_id IN ('" . implode("','", array_keys($this->option_cache)) . "')"
        );
    ...
    function import()
    {
        ...
        $query = $this->old_db->query(
			...
			WHERE g.group_id NOT IN(3,6,7) AND g.forum_id > 0 AND f.forum_type=1 AND (auth_option_id IN ('" . implode(
                "','",
                array_keys($this->option_cache)
            ) . "') OR auth_role_id IN ('" . implode("','", $this->role_cache) . "'))
