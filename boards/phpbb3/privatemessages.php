...
class PHPBB3_Converter_Module_Privatemessages extends Converter_Module_Privatemessages
{
    ...
    function convert_data($data)
    {
        ...
        $insert_data['subject'] = encode_to_utf8(
            utf8_unhtmlentities($data['message_subject']),
            "privmsgs",
            "privatemessages"
        );
        $insert_data['dateline'] = $data['message_time'];
        $insert_data['message'] = encode_to_utf8(
            $this->bbcode_parser->convert($data['message_text'], $data['bbcode_uid']),
            "privmsgs",
            "privatemessages"
        );
        ...
        $tquery = $this->old_db->simple_select(
            'privmsgs_to',
            'msg_id',
            "msg_id={$data['msg_id']} AND user_id={$data['author_id']} AND pm_deleted=0"
        );
