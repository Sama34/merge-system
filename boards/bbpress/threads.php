...
class BBPRESS_Converter_Module_Threads extends Converter_Module_Threads
{
    ...
    function import(): void
    {
        ...
        while ($thread = $this->old_db->fetch_array($query)) {
            $ipquery = $this->old_db->simple_select(
                "postmeta",
                "meta_value",
                "meta_key='_bbp_author_ip' AND post_id='{$thread['ID']}'"
            );
            ...
            $post = [
                ...
                "subject" => $db->escape_string(
                    encode_to_utf8($this->bbcode_parser->convert_title($thread['post_title']), "posts", "threads")
                ),
                ...
                "message" => $db->escape_string(
                    encode_to_utf8($this->bbcode_parser->convert($thread['post_content']), "posts", "posts")
                ),
    ...
    function convert_data(array $data): array
    {
        ...
        $insert_data['subject'] = encode_to_utf8(
            $this->bbcode_parser->convert_title($data['post_title']),
            "posts",
            "threads"
        );
