...
class BBPRESS_Converter_Module_Posts extends Converter_Module_Posts
{
    ...
    function import()
    {
        ...
        $query = $this->old_db->query(
            "SELECT p.*, t.post_parent AS fid, t.post_title AS subject
        ...
        while ($post = $this->old_db->fetch_array($query)) {
            $ipquery = $this->old_db->simple_select(
                "postmeta",
                "meta_value",
                "meta_key='_bbp_author_ip' AND post_id='{$post['ID']}'"
            );
    ...
    function convert_data($data)
    {
        ...
        $insert_data['subject'] = encode_to_utf8(
            $this->bbcode_parser->convert_title($data['subject']),
            "posts",
            "posts"
        );
        ...
        $insert_data['message'] = encode_to_utf8(
            $this->bbcode_parser->convert($data['post_content']),
            "posts",
            "posts"
        );
    ...
    function after_insert($data, $insert_data, $pid)
    {
        ...
        $db->write_query(
            "UPDATE `" . TABLE_PREFIX . "threads` t SET firstpost=(SELECT MIN(pid) FROM `" . TABLE_PREFIX . "posts` p WHERE t.tid=p.tid)"
        );
    }
