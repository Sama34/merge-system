...
class WBB3_Converter_Module_Users extends Converter_Module_Users
{
    ...
    function pre_setup(): void
    {
        ...
        if (!isset($import_session['enc_options'])) {
            $query = $this->old_db->simple_select(
                ...
                "optionName IN('encryption_method', 'encryption_enable_salting', 'encryption_salt_position', 'encryption_encrypt_before_salting')"
            );
        ...
        if (!isset($import_session['nice_options'])) {
            $query = $this->old_db->simple_select(
                WCF_PREFIX . "user_option",
                "optionID, optionName",
                "optionName IN ('" . implode("','", $this->options) . "')"
            );
            while ($option = $this->old_db->fetch_array($query)) {
    ...
    function import(): void
    {
        ...
        $query = $this->old_db->query(
            "SELECT u.*, w.boardLastVisitTime, w.boardLastActivityTime, w.posts, {$this->fields}, r.groupID AS displaygroup, GROUP_CONCAT(g.groupID) as usergroups
    ...
    function convert_data(array $data): array
    {
        ...
        $insert_data['signature'] = encode_to_utf8(
            $this->bbcode_parser->convert($data['signature']),
            WCF_PREFIX . "user",
            "users"
        );
