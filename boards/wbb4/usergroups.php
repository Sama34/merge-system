...
class WBB4_Converter_Module_Usergroups extends Converter_Module_Usergroups
{
    ...
    function import(): void
    {
        ...
        while ($group = $this->old_db->fetch_array($query)) {
            ...
            $oquery = $this->old_db->simple_select(
                WCF_PREFIX . "user_group_option_value",
                "optionID, optionValue",
                "groupID='{$group['groupID']}' AND optionID IN ('" . implode(
                    "','",
                    array_keys($this->nice_options)
                ) . "')"
            );
            while ($opt = $this->old_db->fetch_array($oquery)) {
                // This will get the last part, eg "admin.general.canUseAcp" becomes "canUseAcp"
                $nicename = substr(
                    $this->nice_options[$opt['optionID']],
                    strrpos($this->nice_options[$opt['optionID']], ".") + 1
                );
