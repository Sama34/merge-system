    ...
    function module_list()
    {
        ...
        foreach ($board->modules as $key => $module) {
            ...
            if (count($dependency_list) > 0) {
                echo "<div class=\"module_description\"><small>{$lang->dependencies}: " . implode(
                        ', ',
                        $dependency_list
                    ) . "</small></div>\n";
            }
            ...
            } elseif ($awaiting_dependencies || in_array($key, $import_session['disabled']) || in_array(
                    $key,
                    $import_session['completed']
                ) && $key != "db_configuration") {
            ...
            if ($dbfile == 'sqlite') {
                $db_info[$dbfile] .= "
					<tr class=\"alt_row\">
						...
						<td class=\"last alt_col\"><input type=\"text\" class=\"text_input\" name=\"config[{$dbfile}][dbname]\" id=\"config_{$dbfile}_dbname\" value=\"" . htmlspecialchars_uni(
                ...
                $db_info[$dbfile] .= "
					<tr class=\"alt_row\">
						...
						<td class=\"last alt_col\"><input type=\"text\" class=\"text_input\" name=\"config[{$dbfile}][dbhost]\" id=\"config_{$dbfile}_dbhost\" value=\"" . htmlspecialchars_uni(
                        $mybb->input['config'][$dbfile]['dbhost']
                    ) . "\" /></td>
					</tr>
					<tr>
						<td class=\"first\"><label for=\"config_{$dbfile}_dbuser\">{$lang->database_user}:</label></td>
						<td class=\"last alt_col\"><input type=\"text\" class=\"text_input\" name=\"config[{$dbfile}][dbuser]\" id=\"config_{$dbfile}_dbuser\" value=\"" . htmlspecialchars_uni(
                        $mybb->input['config'][$dbfile]['dbuser']
                    ) . "\" /></td>
					</tr>
					<tr class=\"alt_row\">
						<td class=\"first\"><label for=\"config_{$dbfile}_dbpass\">{$lang->database_pw}:</label></td>
						<td class=\"last alt_col\"><input type=\"password\" class=\"text_input\" name=\"config[{$dbfile}][dbpass]\" id=\"config_{$dbfile}_dbpass\" value=\"" . htmlspecialchars_uni(
            ...
            if (!isset($board->hide_table_prefix) || $board->hide_table_prefix !== true) {
                $db_info[$dbfile] .= "
					<tr class=\"first\">
						...
						<td class=\"last alt_col\"><input type=\"text\" class=\"text_input\" name=\"config[{$dbfile}][tableprefix]\" id=\"config_{$dbfile}_tableprefix\" value=\"" . htmlspecialchars_uni(
                        $mybb->input['config'][$dbfile]['tableprefix']
                    ) . "\" /></td>
					</tr>
					";
            } else {
                $db_info[$dbfile] .= "
					<input type=\"hidden\" name=\"config[{$dbfile}][tableprefix]\" id=\"config_{$dbfile}_tableprefix\" value=\"" . htmlspecialchars_uni(
                        $mybb->input['config'][$dbfile]['tableprefix']
                    ) . "\" />
				";
            }
...
EOF;
    ...
    function finish_conversion()
    {
        ...
        $this->print_warning(
            $lang->sprintf($lang->finish_whats_next, $config['admin_dir']),
            $lang->finish_whats_next_head
        );
...
<td><input type="radio" name="autorefresh" id="autorefresh_no" value="no" /></td>
    ...
    function calculate_stats($in_progress_stats = true)
    {
        ...
        if ($in_progress_stats == true) {
            echo "<i>" . $lang->sprintf(
                    $lang->stats_in_progress,
                    my_number_format($importing_now),
                    $module->settings['friendly_name'],
                    my_number_format($left),
                    my_number_format($pages)
                ) . "</i><br /><br />";
        } else {
            echo "<i>" . $lang->sprintf(
                    $lang->stats,
                    my_number_format($import_session['total_' . $module_name]),
                    $module->settings['friendly_name']
                ) . "</i><br /><br />";
        }
        flush();
    ...
    function update_progress_bar($left, $status_message = "")
    {
        ...
        if ($status_message) {
            echo " document.getElementById('status_message').innerHTML='" . str_replace(
                    "'",
                    "\\'",
                    $status_message
                ) . "';";
        ...
        if ($position == "start") {
            ...
            if ($import_session[$module_name . '_per_screen'] <= 1000 || ($import_session[$module_name . '_per_screen'] > 1000 && ($this->_internal_counter % $modulus) == 0)) {
                ...
                if ($position == "merge_user") {
                    $status_message = $lang->sprintf(
                        $lang->progress_merging_user,
                        $id['import_uid'],
                        $id['duplicate_uid']
                    );
                } else {
                    ...
                    if ($import_session['module'] == "settings") {
                        $status_message = $lang->sprintf(
                            $lang->progress_settings,
                            $this->_friendly_name_singular,
                            $this->_current_id,
                            $module->board->plain_bbname
                        );
                    } else {
                        if (!is_numeric($this->_current_id)) {
