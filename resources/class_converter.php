...
abstract class Converter
{
    ...
    function db_configuration()
    {
        ...
        if ($mybb->input['dbengine']) {
            ...
            if (strstr($mybb->input['dbengine'], "sqlite") !== false && (strstr(
                        $config_data['dbname'],
                        "./"
                    ) !== false || strstr($config_data['dbname'], "../") !== false)) {
                ...
                if (!file_exists(MYBB_ROOT . "inc/db_{$mybb->input['dbengine']}.php")) {
                    ...
                    if (empty($errors)) {
                        ...
                        if (isset($this->check_table) && !empty($this->check_table) && !$this->old_db->table_exists(
                                $this->check_table
                            )) {
                            $errors[] = $lang->sprintf(
                                $lang->error_database_wrong_table,
                                $this->plain_bbname,
                                $config_data['dbname']
                            );
                        }
            ...
            foreach ($t1 as $new_table => $columns) {
                ...
                foreach ($columns as $old_column => $new_column) {
                    ...
                    if (isset($column_info['type']) && ($column_info['type'] == MERGE_DATATYPE_FIXED || $column_info['type'] == MERGE_DATATYPE_BOOL || $column_info['type'] == MERGE_DATATYPE_INT)) {
                        ...
                        if (isset($column_info['type']) && ($column_info['type'] == MERGE_DATATYPE_CHAR || $column_info['type'] == MERGE_DATATYPE_BIN)) {
                    ...
                    if ($percent != $last_percent) {
                        $output->update_progress_bar(
                            $percent,
                            $lang->sprintf($lang->column_length_checking, $old_column, $old_table)
                        );
                    }
    ...
    function check_if_done()
    {
        ...
        if ($import_session['total_' . $module_name] - $this->trackers['start_' . $module_name] <= 0 || $import_session['total_' . $module_name] == 0) {
            ...
            $import_session['flash_message'] = $lang->sprintf(
                $lang->import_successfully,
                $this->settings['friendly_name']
            );
            return true;
        }
