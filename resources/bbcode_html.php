...
class BBCode_Parser_HTML extends BBCode_Parser_Plain
{
    ...
    function convert(string $text): string
    {
        ...
        $text = preg_replace(
            '#<p style="text-align: ?(left|center|right|justify);?">(.*?)</p>#si',
            "[align=$1]$2[/align]\n",
            $text
        );
        $text = preg_replace(
            '#<div style="text-align: ?(left|center|right|justify);?">(.*?)</div>#si',
            "[align=$1]$2[/align]\n",
            $text
        );
        $text = preg_replace(
            '#<span style="color: ?([a-zA-Z]*|\#?[\da-fA-F]{3}|\#?[\da-fA-F]{6});?">(.*?)</span>#si',
            "[color=$1]$2[/color]\n",
            $text
        );
        $text = preg_replace(
            '#<span style="font-family: ?([a-z0-9 ,\-_\'"]+);?">(.*?)</span>#si',
            "[font=$1]$2[/font]\n",
            $text
        );
        ...
        $text = preg_replace(
            '#<span style="font-size: ?(xx-small|x-small|small|medium|large|x-large|xx-large);?">(.*?)</span>#si',
            "[size=$1]$2[/size]\n",
            $text
        );
