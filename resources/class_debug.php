...
class Log
{
                    ...
                    if (!preg_match('//u', $log_insert['message'])) {
                        if (function_exists('mb_convert_encoding')) {
                            ...
                            while (!empty($log_insert['message']) && !preg_match('//u', $log_insert['message'])) {
                                $log_insert['message'] = substr(
                                    $log_insert['message'],
                                    0,
                                    strlen($log_insert['message']) - 1
                                );
                            }
                        }
                    }
    ...
    private function create_debug_table(): void
    {
        ...
        if (!$db->table_exists("debuglogs")) {
            switch ($db->type) {
                case "sqlite":
                    $db->write_query(
                        "CREATE TABLE " . TABLE_PREFIX . "debuglogs (
    ...
    public function __destruct()
    {
        ...
        $this->trace0(
            "Generated in {$total_time} seconds ({$percentphp}% PHP / {$percentsql}% MySQL) / Initialize Load Time: {$load_time} / SQL Queries: {$db->query_count}{$memory_usage} PHP version: {$phpversion} / Server Load: {$serverload}"
        );
    }
}
