...
abstract class Converter_Module_Users extends Converter_Module
{
    ...
    public function check_for_duplicates(&$user)
    {
        ...
        $encoded_username = encode_to_utf8(
            $user[$this->settings['username_column']],
            $this->settings['encode_table'],
            "users"
        );
        ...
        if (strtolower($duplicate_user['username']) == strtolower($username) || my_strtolower(
                $duplicate_user['username']
            ) == strtolower($encoded_username)) {
