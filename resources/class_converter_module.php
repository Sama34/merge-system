...
abstract class Converter_Module
{
    ...
    public function __construct(Converter $converter_class)
    {
        ...
        if ($converter_class->parser_class == "plain" || ($converter_class->parser_class != "html" && !file_exists(
                    MERGE_ROOT . "boards/{$import_session['board']}/bbcode_parser.php"
                ))) {
            $this->bbcode_parser = new BBCode_Parser_Plain();
        } // Using the HTML class? No need for extra checks
        else {
            if ($converter_class->parser_class == "html") {
        ...
        foreach ($data as $key => $value) {
            ...
            if (!empty($column_info[$key]['type'])) {
                ...
                $this->board->set_column_warning_in_progress(
                    ...
                    $key,
                    $lang->sprintf(
                        $lang->warning_prepare_data_unknown_column,
                ...
                if (isset($column['type']) && $column['type'] == MERGE_DATATYPE_BIN) {
                    ...
                    if ($value_length > $limit) {
                        ...
                        $this->board->set_column_warning_in_progress(
                            ...
                            $key,
                            $lang->sprintf(
                                $lang->warning_prepare_data_data_truncation_binary,
                                $column['def_type'],
                                $value_length,
                                $limit
                            ),
                            $lang->sprintf(
                                $lang->warning_prepare_data_data_truncated,
                                TABLE_PREFIX . $table,
                                $key,
                                $column['def_type'],
                                'The input BINARY',
                                var_export(bin2hex($value_original), true),
                                var_export(bin2hex($value), true)
                            )
                        );
                    }
                } else {
                    $this->board->set_column_warning_in_progress(
                        ...
                        $key,
                        $lang->sprintf(
                            $lang->warning_prepare_data_mismatched_column,
                            $import_session['module'],
                            TABLE_PREFIX . $table,
                            $key,
                            $column['def_type'],
            ...
            // Actual column type varies, could be *int, numeric/decimal, etc.
                ...
                if (isset($this->integer_fields) && in_array($key, $this->integer_fields)) {
                    ...
                    if (!is_numeric($value)) {
                        ...
                        $this->board->set_column_warning_in_progress(
                            ...
                            $lang->sprintf(
                                $lang->warning_prepare_data_data_casted,
                                TABLE_PREFIX . $table,
                                $key,
                                $column['def_type'],
                                'The input',
                                'INTEGER',
                                var_export($value_original, true),
                    ...
                    if (isset($column['type']) && ($column['type'] == MERGE_DATATYPE_BOOL || isset($column['type_table']) && $column['type_table'] == MERGE_DATATYPE_BOOL)) {
                        ...
                        if (isset($column['type']) && ($column['type'] == MERGE_DATATYPE_INT || $column['type'] == MERGE_DATATYPE_FIXED)) {
                            ...
                            if ($column['type'] == MERGE_DATATYPE_FIXED) {
                                ...
                                if (isset($column['min_table'], $column['max_table'])) {
                            ...
                            if ($int_is_truncated) {
                                $this->board->set_column_warning_in_progress(
                                    ...
                                    $key,
                                    $lang->sprintf(
                                        $lang->warning_prepare_data_data_truncation_integer,
                                        $column['def_type'],
                                        $value_original,
                                        $int_limit
                                    ),
                    ...
                    if (isset($column['type']) && $column['type'] == MERGE_DATATYPE_CHAR) {
                        ...
                        if ($limit_type == MERGE_DATATYPE_CHAR_LENGTHTYPE_CHAR && $value_char_length > $limit) {
                            ...
                            $this->board->set_column_warning_in_progress(
                                ...
                                $key,
                                $lang->sprintf(
                                    $lang->warning_prepare_data_data_truncation_string,
                                    $column['def_type'],
                                    $value_char_length,
                                    $limit,
                                    'char'
                                ...
                                $lang->sprintf(
                                    ...
                                    $key,
                                    $column['def_type'],
                                    'The input STRING (by char)',
                                    var_export($value_original, true),
                                    var_export($value, true)
                                )
                            );
                        } else {
                            if ($limit_type == MERGE_DATATYPE_CHAR_LENGTHTYPE_BYTE && $value_byte_length > $limit) {
