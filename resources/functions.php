    ...
    foreach ($drop_list as $table => $columns) {
        if ($text == true) {
            ...
            $output->update_progress_bar(
                $progress,
                $lang->sprintf($lang->removing_columns, $columns_list, TABLE_PREFIX . $table)
            );
            $progress += $increment;
        }
...
function create_import_fields($text = true)
{
    ...
    foreach ($add_list['int'] as $table => $columns) {
        if ($text == true) {
            ...
            $output->update_progress_bar(
                $progress,
                $lang->sprintf($lang->creating_columns, "int", $columns_list, TABLE_PREFIX . $table)
            );
            $progress += $increment;
        }
        ...
        if (!empty($columns_to_add)) {
            if ($db->type == "sqlite") {
                ...
                foreach ($columns_to_add as $column) {
                    $db->write_query(
                        "ALTER TABLE " . TABLE_PREFIX . $table . " ADD {$column} int NOT NULL DEFAULT '0'"
                    );
                }
            } else {
                $columns_sql = implode(" int NOT NULL default '0', ADD ", $columns_to_add);
                $db->write_query(
                    "ALTER TABLE " . TABLE_PREFIX . $table . " ADD {$columns_sql} int NOT NULL DEFAULT '0'"
                );
    ...
    foreach ($add_list['text'] as $table => $columns) {
        if ($text == true) {
            ...
            $output->update_progress_bar(
                $progress,
                $lang->sprintf($lang->creating_columns, "text", $columns_list, TABLE_PREFIX . $table)
            );
            $progress += $increment;
        }
...
function encode_to_utf8($text, $old_table_name, $new_table_name)
{
    ...
    if (($import_session['table_charset_new'][$new_table_name] != $import_session['table_charset_old'][$old_table_name]
            || check_encoding(
                $text,
                fetch_iconv_encoding($import_session['table_charset_new'][$new_table_name])
            ) === false)
        && $import_session['table_charset_old'][$old_table_name] != ''
        && $import_session['table_charset_new'][$new_table_name] != '') {
        if (!function_exists('iconv')) {
            if (fetch_iconv_encoding(
                    $import_session['table_charset_old'][$old_table_name]
                ) != 'iso-8859-1' || !function_exists("utf8_encode")) {
                return $text;
            }
        ...
        $converted_str = iconv(
            fetch_iconv_encoding($import_session['table_charset_old'][$old_table_name]),
            fetch_iconv_encoding($import_session['table_charset_new'][$new_table_name]) . '//TRANSLIT',
            $text
        );
        ...
        if (my_strlen($converted_str) < my_strlen($text)) {
            ...
            $converted_str = iconv(
                "iso-8859-1",
                fetch_iconv_encoding($import_session['table_charset_new'][$new_table_name]) . '//IGNORE',
                $text
            );
            if (my_strlen($converted_str) >= my_strlen($text)) {
...
function merge_fetch_remote_file($url, $post_data = array())
{
    ...
    if (my_substr($url, 0, 1) == '.' || my_substr($url, 0, 1) == '/' || my_substr($url, 1, 2) == ':\\' || my_substr(
            $url,
            1,
            2
        ) == ':/') {
    ...
    foreach ($columns as $column) {
        ...
        if ($db->type == 'sqlite') {
            ...
            if (strpos($column_type, 'int') !== false) {
                ...
                if (strpos($column_type, 'char') !== false || strpos($column_type, 'clob') !== false || strpos(
                        $column_type,
                        'text'
                    ) !== false) {
                    ...
                    if (!is_null(
                            $column_type_arg
                        ) && ($column_type_table == 'char' || $column_type_table == 'varchar')) {
                        ...
                        if (strpos($column_type_table, 'text') !== false) {
                            switch ($column_type_table) {
                                case 'tinytext':
                    ...
                    if (strpos($column_type, 'blob') !== false || empty($column_type)) {
                        ...
                        if (strpos($column_type, 'real') !== false || strpos($column_type, 'floa') !== false || strpos(
                                $column_type,
                                'doub'
                            ) !== false) {
                            ...
                            if (!is_null(
                                    $column_type_arg
                                ) && ($column_type_table == 'numeric' || $column_type_table == 'decimal')) {
        ...
        // The boolean type that is equivalent to TINYINT(1) in MySQL and boolean in PostgreSQL.
            ...
            if ($column_type == 'tinyint(1)' || $column_type == 'boolean') {
                    ...
                    if ($column_type == 'bit' || $column_type == 'bit varying') {
                        ...
                        if (strpos($column_type, 'int') !== false) {
                            ...
                            if ($column_type == 'decimal' || $column_type == 'numeric') {
                                ...
                                if ($column_type == 'float' || $column_type == 'real') {
                                    ...
                                    if ($column_type == 'double' || $column_type == 'double precision') {
                                        $columninfo['type'] = MERGE_DATATYPE_DBL;
                                        ...
                                        if (strpos($column_type, 'char') !== false || strpos(
                                                $column_type,
                                                'text'
                                            ) !== false) {
                                            $columninfo['type'] = MERGE_DATATYPE_CHAR;
                                            ...
                                            if (!is_null($column_type_arg)) {
                                                ...
                                                if (strpos(
                                                        $column_type,
                                                        'text'
                                                    ) !== false && ($db->type == 'mysql' || $db->type == 'mysqli')) {
                                                    switch ($column_type) {
                                                        case 'tinytext':
                                                            $char_type_length = MERGE_DATATYPE_CHAR_TINYTEXT_LENGTH;
                                                            break;
                                            ...
                                            if (strpos($column_type, 'binary') !== false || strpos(
                                                    $column_type,
                                                    'blob'
                                                ) !== false || $column_type == 'bytea') {
                                                $columninfo['type'] = MERGE_DATATYPE_BIN;
