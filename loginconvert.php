...
$valid_login_types = [
    ...
    "ipb4" => "ipb4",
    "smf" => "smf",
    // Isn't supported anymore, but the function is still required by smf 1.1 and 2 and there may be "old" users
    "smf11" => "smf11",
    "smf2" => "smf2",
    "punbb" => "punbb",
    "phpbb3" => "phpbb3",
    "bbpress" => "bbpress",
    "xf" => "xf11",
    // XenForo can have two different authentications
    "xf12" => "xf12",
    // 1.2+ use PHP's crypt function by default
    "wbb3" => "wcf1",
    // WBB 3 and Lite 2 use WoltLab Community Framework 1.x with some special parameters
    "wbb4" => "wcf2",
    // WBB 4 uses WoltLab Community Framework 2.x
    "vanilla" => "vanilla",
    "fluxbb" => "punbb",
...
function loginconvert_activate(): void
{
    ...
    if (!$db->field_exists("passwordconvert", "users")) {
        flash_message(
            "There's no need to activate this plugin as there aren't any passwords which need to be converted",
            "error"
        );
...
function loginconvert_convert(LoginDataHandler &$login): void
{
    ...
    if (!array_key_exists($user['passwordconverttype'], $valid_login_types)) {
        ...
        redirect(
            ...
            "We're sorry but we couldn't convert your old password. Please select a new one",
            "",
            true
        );
    } else {
        ...
        if (!$check && in_array($login_type, $utf8_recheck) && utf8_decode(
                $login->data['password']
            ) != $login->data['password']) {
...
function check_smf(string $password, array $user): bool
{
    if (crypt($password, substr($password, 0, 2)) == $user['passwordconvert']) {
        ...
        if (my_strlen($user['passwordconvert']) == 32 && md5_hmac(
                preg_replace("#\_smf1\.1\_import(\d+)$#i", '', $user['username']),
                $password
            ) == $user['passwordconvert']) {
            return true;
        } else {
            if (my_strlen($user['passwordconvert']) == 32 && md5($password) == $user['passwordconvert']) {
                return true;
            }
        }
    }
...
function check_smf11(string $password, array $user): bool
{
    ...
    if ($is_sha1 && sha1(
            strtolower(preg_replace("#\_smf1\.1\_import(\d+)$#i", '', $user['username'])) . $password
        ) == $user['passwordconvert']) {
        return true;
    } else {
...
function check_smf2(string $password, array $user): bool
{
    ...
    if ($is_sha1 && sha1(
            strtolower(preg_replace("#\_smf2\.0\_import(\d+)$#i", '', $user['username'])) . $password
        ) == $user['passwordconvert']) {
        return true;
    } else {
        return check_smf($password, $user);
...
function check_punbb(string $password, array $user): bool
{
    ...
    if (function_exists('sha1') && $is_sha1 && (sha1($password) == $user['passwordconvert'] || sha1(
                $user['passwordconvertsalt'] . sha1($password)
            ) == $user['passwordconvert'])) {
        return true;
    } elseif (function_exists('mhash') && $is_sha1 && (bin2hex(
                mhash(MHASH_SHA1, $password)
            ) == $user['passwordconvert'] || bin2hex(
                mhash(MHASH_SHA1, $user['passwordconvertsalt'] . bin2hex(mhash(MHASH_SHA1, $password)))
            ) == $user['passwordconvert'])) {
        return true;
    } else {
...
function check_wcf1(string $password, array $user): bool
{
    ...
    if (wcf1_encrypt(
            $user['passwordconvertsalt'] . wcf1_hash($password, $user['passwordconvertsalt'], $settings),
...
function check_vanilla(string $password, array $user): bool
{
    if ($user['passwordconvert'][0] === '_' || $user['passwordconvert'][0] === '$') {
        ...
        if ($password && $user['passwordconvert'] !== '*' && ($password === $user['passwordconvert'] || md5(
                    $password
                ) === $user['passwordconvert'])) {
            return true;
        }
